    ...
    def run(self, num_d: int, word_d: list) -> None:  # 핸들러 실행
        ...
        for i in range(1, int(num_d) + 1):
            ...
            element = WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable((By.XPATH, f"//*[@id='testForm']/div[{i}]/div/div[1]/div[2]/div[2]/div/div"))
            )
            element.click()
            ...
            try:
                ...
                WebDriverWait(driver, 10).until(
                    EC.element_to_be_clickable((By.XPATH, f"/html/body/div[2]/div/div[2]/div[2]/form/div[{i}]/div/div[2]/div/div[1]"))
                )
                ...
                for box_item in box_items:
                    lenght = 6
                    if len(text) >= lenght :
                        if box_item.text[:lenght] == text[:lenght]:
                            box_item.click()
                            break
                    else:
